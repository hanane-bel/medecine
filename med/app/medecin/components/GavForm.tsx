...
export default function GavForm({ patientId, patientName, patientData }: GavFormProps) {
    ...
    const saveSchema = () => {
        ...
        if (canvas) {
            ...
            setField(patientId, "gav_examen", "schema_data", dataUrl);
            setField(patientId, "gav_certificat", "schema_data", dataUrl);
    ...
    useEffect(() => {
        let schemaData = "";
        const savedData1 = getFormData(patientId, "gav_examen");
        const savedData2 = getFormData(patientId, "gav_certificat");
        
        if (savedData1 && savedData1["schema_data"]) {
            schemaData = savedData1["schema_data"];
        } else if (savedData2 && savedData2["schema_data"]) {
            schemaData = savedData2["schema_data"];
    ...
    }, [patientId, patientData, getFormData, currentView]);
    ...
    const handleDownloadExamPDF = async () => {
        ...
        try {
            ...
            pdf.save(`Fiche_Examen_GAV_${patientName.replace(/\s+/g, '_')}.pdf`);
        } catch (error: any) {
    ...
    const handleDownloadCertificatPDF = async (selectedSections: string[]) => {
        ...
        try {
            ...
            pdf.addImage(imgData, 'JPEG', 0, 0, pdfWidth, pdfHeight);
            pdf.save(`Certificat_GAV_${patientName.replace(/\s+/g, '_')}.pdf`);
        } catch (error: any) {
            console.error("PDF Generation Error (Certificat):", error);
            alert("Erreur lors de la génération du PDF: " + (error?.message || error));
        } finally {
            formRef.current.classList.remove("pdf-mode");
            showAllFields();
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            const img = new Image();
            img.crossOrigin = "Anonymous"; // Ensure no tainted canvas issues
            img.onload = () => {
                const canvas = schemaCanvasRef.current;
                if (!canvas) return;
                const ctx = canvas.getContext("2d");
...
`}</style>
                    ...
                    <fieldset disabled={!isEditing} className="contents group">
                        <div className="space-y-6 print:space-y-2">
                            ...
                            <div className="print:break-before-page" style={{ display: shouldShowExamen("schema") && (schemaHasContent || !showAllForPrint) ? 'block' : 'none' }}>
...
`}</style>
                ...
                <fieldset disabled={!isEditing} className="contents group">
                    <div className="space-y-3 text-[11pt] leading-relaxed relative print-container bg-white/5 text-white print:bg-white print:text-black border border-white/10 print:border-none rounded-xl print:rounded-none p-6 print:p-0 shadow-none print:shadow-none mt-4 print:mt-0">
                        ...
                        <div className="print:break-before-page" style={{ display: shouldShowCertificat("schema_c") && (schemaHasContent || !(showAllForPrint && isPrintingReport)) ? 'block' : 'none' }}>
